...
export const handleGetPortfolios = (payload) => {
  ...
  return dispatch => {
    fetch(`${HOST}getportfolio`, options)
      .then(res => res.json())
      .then(res => {
        if (res.code === 200) {
          portfolios = { ...portfolios, ...res.portfolios }; // TODO: remove this
          dispatch({ type: HANDLE_GET_PORTFOLIO_SUCCESS, portfolios });
        } else {
          dispatch({ type: HANDLE_GET_PORTFOLIO_FAILURE, text: res.body.text });
        }
      })
      .catch(err => {
        dispatch({ type: HANDLE_GET_PORTFOLIO_FAILURE, text: ERROR_UNEXPECTED });
      });
  }
};
